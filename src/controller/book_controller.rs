...
use crate::{config::db::Pool, model::{book::BookDTO, custom_errors::MyError, dtos::FullBookDTO, traits::Saveable}};
...
#[post("/")]
pub async fn insert_book(new_book: Json<NewBook>, pool: Data<Pool>) -> Result<BookDTO, MyError> {
    new_book.0.save(&mut pool.get().unwrap())
}